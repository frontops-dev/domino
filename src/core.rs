...
use crate::types::{AffectedResult, Project, TrueAffectedConfig};
use crate::utils;
...
pub fn find_affected(
  ...
  // Step 4: Track affected packages
  ...
  add_implicit_dependencies(&config.projects, &mut affected_packages);
  ...
  Ok(AffectedResult { affected_projects })
}
...
fn add_implicit_dependencies(projects: &[Project], affected_packages: &mut FxHashSet<String>) {
...
mod tests {
  ...
  fn test_add_implicit_dependencies() {
    ...
    add_implicit_dependencies(&projects, &mut affected);
