...
pub fn is_turbo_workspace(cwd: &Path) -> bool {
  cwd.join("turbo.json").exists()
    || cwd.join("pnpm-workspace.yaml").exists()
    || has_npm_workspaces(cwd)
}
...
pub fn get_projects(cwd: &Path) -> Result<Vec<Project>> {
  ...
  debug!("Found {} Turbo projects", projects.len());
  Ok(projects)
}

fn get_workspace_patterns(cwd: &Path) -> Result<Vec<String>> {
...
fn parse_package_json(path: &Path, cwd: &Path) -> Result<Project> {
  ...
  Ok(Project {
    name: pkg_json.name,
    source_root,
    ts_config: None, // Turbo doesn't specify tsconfig in the same way
